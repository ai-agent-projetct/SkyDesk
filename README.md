...
6. The homepage simulator video (`public/media/sim-demo.mp4` + `.jpg` poster) is recorded from the live 3D scene at `/demo/field`.
   To re-record, open `/demo/field?record=1&bps=2500000` in Chrome — it renders one seamless 31.6 s loop frame by frame (WebCodecs, works
   even in a background tab) and downloads `aeron-simulator.mp4`; replace `public/media/sim-demo.mp4` with it.
