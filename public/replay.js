...
(function () {
  ...
  const MODE_COL = ['#38bdf8', '#f59e0b', '#22c55e', '#a78bfa', '#f43f5e', '#2f7bf5', '#eab308', '#fb7185', '#60a5fa', '#84cc16'];
  ...
  function draw() {
    ...
    ctx.fillStyle = '#0c1b52'; ctx.fillRect(0, 0, cv.width, cv.height);
    // ground grid
  ...
  function drawProfile() {
    ...
    pctx.fillStyle = 'rgba(13,94,241,.12)'; pctx.strokeStyle = '#0d5ef1'; pctx.lineWidth = 1.5 * dpr; pctx.beginPath();
  ...
  function sticks(c) { // Mode 2: left = throttle (up) / yaw, right = pitch / roll
    ...
    [[W * 0.27, c.yaw, c.thr != null ? 1 - 2 * c.thr : null], [W * 0.73, c.rol, c.pit]].forEach(([cx, sx, sy]) => {
      ...
      g.fillStyle = '#0d5ef1'; g.beginPath(); g.arc(cx + (sx || 0) * r, H / 2 + (sy || 0) * r, 7, 0, 7); g.fill();
    });
  ...
  const LINE_COL = ['#0d5ef1', '#f59e0b', '#16a34a', '#db2777'];
