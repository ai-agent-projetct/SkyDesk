...
// Colours: body = fuselage, arms and skids; wing = propeller blades and guards.
import * as THREE from 'three';

const DEG = Math.PI / 180;
export function buildDrone(bodyColor, canopyColor = 0x1e293b, wingColor = 0x0b1220) {
  ...
  const accent = M(0x2774ed, { roughness: 0.4 }), black = M(0x0b1220, { roughness: 0.2, metalness: 0.7 }), wing = M(wingColor, { roughness: 0.3, metalness: 0.3 });
  ...
  [45, 135, 225, 315].forEach((a, i) => {
    ...
    const arm = add(new THREE.CylinderGeometry(0.03, 0.038, L, 10), body, ax * L / 2, 0.02, az * L / 2);
    ...
    for (const s of [0, Math.PI]) add(new THREE.BoxGeometry(0.46, 0.008, 0.045), wing, 0, 0, 0, hub).rotation.y = s;
    const disc = add(new THREE.CircleGeometry(0.25, 32), new THREE.MeshBasicMaterial({ color: wingColor, transparent: true, opacity: 0, depthWrite: false }), 0, 0.002, 0, hub);
    disc.rotation.x = -Math.PI / 2;                                                                         // motion blur disc when spinning fast
    const guard = add(new THREE.TorusGeometry(0.28, 0.012, 8, 40), wing, mx, 0.115, mz); guard.rotation.x = Math.PI / 2;
  ...
  for (const x of [-0.16, 0.16]) {                                                                          // skids
    add(new THREE.CylinderGeometry(0.014, 0.014, 0.5, 8), body, x, -0.27, 0).rotation.x = Math.PI / 2;
    for (const z of [-0.12, 0.12]) add(new THREE.CylinderGeometry(0.012, 0.012, 0.16, 8), body, x * 0.8, -0.19, z).rotation.z = x > 0 ? -0.35 : 0.35;
  }
